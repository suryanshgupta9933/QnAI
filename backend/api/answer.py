...
def create_answer_endpoint(answer: CreateAnswer):
    """
    Create a answer to a post by a user within a department.
    """
    try:
        answer_id = str(uuid.uuid4())
        create_answer(answer.org_id, answer.user_id, answer.post_id, answer_id, answer.content)
        return {
            "answer_id": answer_id,
...
def get_answers_endpoint(answer: GetAnswer):
    """
    Retrieve all answers to a post by a user.
    """
    try:
        answers = get_answers(answer.org_id, answer.user_id, answer.post_id)
        if answers:
            return answers
        else:
...
def update_answer_votes_endpoint(update: UpdateAnswerVotes):
    try:
        update_answer_votes(update.org_id, update.user_id, update.post_id, update.upvotes, update.downvotes)
        return {
